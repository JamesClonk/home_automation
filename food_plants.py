...
def update():
    ...
    print "curl -X POST -d 'value={0:0d}' -u {1}:{2} https://home-info.jamesclonk.io/sensor/{3}/value".format(int(moisture_three), username, password, soil_id_three)
    os.system("curl -X POST -d 'value={0:0d}' -u {1}:{2} https://home-info.jamesclonk.io/sensor/{3}/value".format(int(moisture_three), username, password, soil_id_three))
    ...
    if moisture_one < 74 or moisture_two < 74:
        currentDT = datetime.datetime.now()
        if currentDT.hour > 6 and currentDT.hour < 8 and currentDT.minute < 8:
            water()
