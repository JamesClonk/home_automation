...
const String sensorIdPa = "30";    // gallery air pressure
const String sensorIdAlt = "29";   // gallery altitude
const String sensorIdTemp = "25";  // gallery temperature
const String sensorIdHum = "26";   // gallery humidity
...
void setup() {
  ...
  if (!sht4.begin(&Wire, SHT40_I2C_ADDR_44, 2, 1, 400000U)) {
    silentMode = false;
    permanentError = true;
    reportError("could not find SHT40!");
    while (true) delay(1000);  // block endlessly here, no point in continuing if SHT40 was not found! Hard reset of AtomS3 device required!
  }
  ...
  if (!bmp.begin(&Wire, BMP280_I2C_ADDR, 2, 1, 400000U)) {
    silentMode = false;
    permanentError = true;
    reportError("could not find BMP280!");
    while (true) delay(1000);  // block endlessly here, no point in continuing if BMP280 was not found! Hard reset of AtomS3 device required!
  }
...
void runCollection() {
  ...
  if (measurement) {
    ...
    if (httpUploadEnabled) {
      ...
      upload = httpUpload(sensorIdPa, hpa) && upload;
      upload = httpUpload(sensorIdAlt, alt) && upload;
